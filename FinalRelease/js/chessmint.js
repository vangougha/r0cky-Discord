...
class StockfishEngine {
    constructor(selfmaster) {
        ...
        this.threads = this.selfmaster.options.threads;
    ...
    onTopMoves(move = null, isBestMove = false) {
        ...
        if (this.selfmaster.options.auto_move && isBestMove && this.selfmaster.game.controller.getPlayingAs() == this.selfmaster.game.controller.getTurn()) {
            let bestMove = top_pv_moves[0];
            let legalMoves = this.selfmaster.game.controller.getLegalMoves();
...
function InitChessMint(chessboard) {
    ...
    ChromeRequest.getData().then(function (options) {
        try {
            selfmaster = new ChessMint(chessboard, options);
        }
        catch (e) {
            console.error(e);
            alert("Failed to load Chess selfmaster");
        }
    });
}
